...
class KernelCAMRegressors(BaseEstimator):
    def __init__(self, 
        ...
        parameter_dict: (default is `None`) a dictionary of parameters of the basic estimators given in the `estimator_list` argument. It should be a dictionary containing the names of the basic estimators, 
    ...
    def build_baisc_estimators(self):
        ...
        self.estimator_names = estimator_dict
        param_dict = {
            'knn' : None,
            'lasso' : None,
            'ridge' : None,
        ...
        if self.parameter_dict is not None:
            for name in param_dict:
                if name in self.parameter_dict:
                    param_dict[name] = self.parameter_dict[name]
        for machine in estimator_dict:
            try:
                mod = estimator_dict[machine]
                if param_dict[machine] is not None:
                    mod.set_params(params = param_dict[machine])
